...
from utils.loader import load_seed, load_device, load_data, load_model_params, load_model_optimizer, \
                         load_ema, load_loss_fn, load_batch, load_control_net, load_batch_ddi
...
class MultiDataControlTrainer(object):
    def __init__(self, config):
        ...
        self.ddi_label_embedding = TransDDI(self.config.controller, self.params_x, self.params_adj, f'cuda:{self.device[0]}', self.config.train)
        self.ddi_label_embedding.load_state_dict(torch.load(self.config.controller.cldr_ckpt)['model_state_dict'], strict = True)
        self.ddi_label_embedding.to('cuda:'+str(self.device[0]))
        self.ddi_label_embedding.eval()
    ...
    def train(self, ts):
        ...
        if self.config.train.resume is False and self.config.train.pretrain:
        ...
        elif self.config.train.pretrain:
        ...
        for epoch in trange(0, (self.config.train.num_epochs), desc = '[Epoch]', position = 1, leave=False):
            ...
            for _, train_b in enumerate(self.train_loader):
                ...
                x1, adj1, x2, adj2, label, s1, s2 = load_batch_ddi(train_b, self.device) 
                
                loss_subject = (x1, adj1, self.ddi_label_embedding.multi_data_text_forward(x1, adj1, x2, adj2, label, s1, s2))
