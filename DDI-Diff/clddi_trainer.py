...
from utils.loader import load_seed, load_device, load_data, load_model_params, load_model_optimizer, \
                         load_ema, load_loss_fn, load_batch_ddi
    ...
    def train(self, ts):
        ...
        for epoch in trange(0, (self.config.train.num_epochs), desc = '[Epoch]', position = 1, leave=False):
            ...
            for _, train_b in tqdm(enumerate(self.train_loader)):
                ...
                logits_per_dc, logits_per_text = self.model.forward4gen(x1, adj1, x2, adj2, label, s1, s2)
                labels = torch.arange(label.shape[0]).long().to(f'cuda:{self.device[0]}')
            ...
            with torch.no_grad():
                for _, test_b in enumerate(self.test_loader):   
                    ...
                    x1, adj1, x2, adj2, label, s1, s2 = load_batch_ddi(test_b, self.device) 
                    logits_per_dc, logits_per_text = self.model.forward4gen(x1, adj1, x2, adj2, label, s1, s2)
                    labels = torch.arange(label.shape[0]).long().to(f'cuda:{self.device[0]}')
